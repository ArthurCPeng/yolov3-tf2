...
def conv_block(input_data, kernel, filters, name,
    ...
    conv = layers.Conv2D(filters, kernel,
...
def res_block(input_data, filters1, filters2, name, trainable=True):
    ...
    input_data = conv_block(input_data, (1,1), filters1, name=name+'_conv1')
    input_data = conv_block(input_data, (3,3), filters2, name=name+'_conv2')

    return input_data + shortcut
    
    
def darknet53(input_data, trainable=True):
    input_data = conv_block(input_data, (3,3), factor * 2, trainable=trainable, name='conv1')
    input_data = conv_block(input_data, (3,3), factor * 4, trainable=trainable, name='conv2',
                            downsample = True)
    ...
    input_data = conv_block(input_data, (3,3), factor * 8, trainable=trainable, name='conv3',
                            downsample = True)
    ...
    input_data = conv_block(input_data, (3,3), factor * 16, trainable=trainable, name='conv4',
                            downsample = True)
    ...
    route1 = input_data
    input_data = conv_block(input_data, (3,3), factor * 32, trainable=trainable, name='conv5',
                            downsample = True)
    ...
    route2 = input_data
    input_data = conv_block(input_data, (3,3), factor * 64, trainable=trainable, name='conv6',
                            downsample = True)
    for i in range(4):
...
def upsample_block(input_data, name, method='deconv'):
    ...
    if method == 'resize':
        ...
        '''
        output = layers.Upsample2D(size=(2,2))(input_data)
        
    if method == 'deconv':
        output = layers.Conv2DTranspose(input_shape[-1], kernel_size = 2,
                                        strides=(2,2), name = name + '_deconv')(input_data)
    return output
